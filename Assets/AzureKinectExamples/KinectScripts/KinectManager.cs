...
namespace com.rfilkov.kinect
{
    ...
    public class KinectManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Gets all user body data (do not change the data in the structur)
        /// </summary>
        ...
        public Vector3 GetPosColorOverlay(Vector3 posSensorSpace, int sensorIndex, Rect imageRect)
        {
            if (posSensorSpace != Vector3.zero)
            {
                ...
                if (posColor.x != 0f && !float.IsInfinity(posColor.x))
                {
                    ...
                    //Debug.Log($"color pos: {posColor:F0}, xSc: {xScaled:F2}, ySc: {yScaled:F2}, rect: {imageRect:F2}\nxImg: {xImage:F2}, yImg: {yImage:F2}");
