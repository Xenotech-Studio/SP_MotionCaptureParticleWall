    ...
    public class Kinect4AzureInterface : DepthSensorBase
    {
        ...
        public override List<KinectInterop.SensorDeviceInfo> GetAvailableSensors()
        {
            ...
            for (int i = 0; i < deviceCount; i++)
            {
                using (Device device = Device.Open(i))
                {
                    ...
                    sensorInfo.sensorId = device.SerialNum;  // i.ToString();
            ...
            if (consoleLogMessages)
            {
                ...
                sbBuf.AppendLine("Available sensors:");

                for (int i = 0; i < deviceCount; i++)
                {
                    sbBuf.Append($"D{i}: ").AppendLine(alSensorInfo[i].ToString());
                }
        ...
        public override KinectInterop.SensorData OpenSensor(KinectManager kinectManager, KinectInterop.FrameSource dwFlags, bool bSyncDepthAndColor, bool bSyncBodyAndDepth)
        {
            ...
            else
            {
                ...
                kinectSensor = Device.Open(alSensors[deviceIndex].deviceIndex);
                if (kinectSensor == null)
                {
                    Debug.LogError($"  D{deviceIndex} - Device {alSensors[deviceIndex].deviceIndex}, id: {alSensors[deviceIndex].sensorId} cannot be opened.");
                    return null;
                }
        ...
        // checks if the sensor is closed or not
        public bool IsSensorClosed()
        {
            ...
            if (detectFloorForPoseEstimation && floorDetector != null)
            {
                if(floorDetector.UpdateFloorDetector(sensorData.depthImage, sensorData.lastDepthFrameTime, ref depthFrameLock, minDepthDistance, maxDepthDistance))
                {
                    lock (poseFrameLock)
                    {
                        ...
                        //Debug.Log($"vSensorPosition: {vSensorPosition}, qSensorRotation: {qSensorRotation.eulerAngles}");
        ...
        private void GetCameraIntrinsics(CalibrationDeviceType camType, CameraCalibration camParams, ref KinectInterop.CameraIntrinsics intr)
        {
            ...
            //string intrStr = string.Join(';', camIntr.Parameters);
            //Debug.Log($"{camType} intr: {intrStr}; Calib: {camIntr.Type}");
        ...
        {
            //string rotStr = string.Join(';', camExtr.Rotation);
            //string transStr = string.Join(';', camExtr.Translation);
            //Debug.Log($"  rot: {rotStr} trans: {transStr}");
